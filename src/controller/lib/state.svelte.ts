...
class AppState {
  ...
  activeTab = $state<'library' | 'setlist' | 'controls'>('library')

  // The song currently expanded/viewed in the controller UI
  focusedSongId = $state<string | null>(null)

  // Search query in the library tab
  searchQuery = $state('')
